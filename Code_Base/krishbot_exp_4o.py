...
ROOT_FOLDER = Path('C:\\Users\\S3580841\\Desktop\\Projs\\Doc_Chat\\Code_base\\data_store')
...
NOTES_FOLDER = Path("C:/Users/S3580841/Desktop/Projs/Doc_Chat/Code_base/Notes")
...
AIRA is now ready to help with technical documentation queries."""




PDF_DIR = Path("C:\\Users\\S3580841\\Desktop\\Projs\\Doc_Chat\\Code_base\\Sample papers\\Actual_papers")
...
endpoint = "https://sg-craft-na-lab-openai-prd.openai.azure.com/"
model_name = "gpt-4o"
deployment = "gpt-4o"

subscription_key = "c14db610df734d99a5ccbab19eb32886"
...
def process_query(user_query):
    """Process the user query and get a response from GPT.""" 
...
def conversation(qa_chain, message):
    ...
    # Retrieve top 3 relevant docs manually
...
User Query: {message}"""
    ...
    # Search for tables
    # show_tables = should_display_table(message) and ("<table" not in response_answer.lower())
    table_outputs = []

    if should_display_table(message):
    ...
    return response_answer, list(references), image_outputs, []
...
def generate_auto_title(first_message):
    ...
    try:
        ...
        title = query_openai(summary_prompt).strip()
    ...
    if uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            if uploaded_file.size > 10 * 1024 * 1024:
                st.warning(f"❌ {uploaded_file.name} exceeds 10MB limit.")
                continue
